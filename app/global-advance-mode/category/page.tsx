...
export default function CategoryPage() {
  ...
  // Fetch single category when editing
  const { data: singleCategoryResponse, isLoading: isLoadingCategory } = useAdvanceCategory(
    editingCategoryId || 0,
    customerId
  )

  // Get the category being edited - use fetched data if available, otherwise fallback to list data
  // singleCategoryResponse is ApiResponse<AdvanceCategory>, so we access .data to get the AdvanceCategory
  const editingCategory = editingCategoryId 
    ? (singleCategoryResponse?.data || categoryData.find((cat) => cat.id === editingCategoryId))
    : null
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
      ...
      <AddCategoryModal
        ...
        isLoading={createCategoryMutation.isPending || updateCategoryMutation.isPending || isLoadingCategory}
