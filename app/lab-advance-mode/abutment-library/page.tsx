...
export default function AbutmentLibraryPage() {
    ...
    updateStatusMutation.mutate(
      { id, status: newStatus },
      {
        onSuccess: () => {
          toast({
            title: "Success",
            description: `Abutment status updated to ${newStatus}`,
          })
        },
        onError: () => {
          toast({
            title: "Error",
            description: "Failed to update abutment status",
            variant: "destructive",
          })
        },
      }
  ...
  const handleDelete = async (id: number) => {
    ...
    deleteAbutmentMutation.mutate(id, {
      onSuccess: () => {
        toast({
          title: "Success",
          description: "Abutment deleted successfully",
        })
      },
      onError: () => {
        toast({
          title: "Error",
          description: "Failed to delete abutment",
          variant: "destructive",
        })
      },
    })
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
      ...
      {/* Pagination */}
      {!isLoading && !isError && abutmentData.length > 0 && (
