...
- Place the waffle launcher as the first item in the left header group, followed immediately by the logo and existing action buttons.
...
- Add a focused test that verifies the shared header declares and renders the waffle launcher before the requested logo.
