                ...
                return Object.entries(groupedByProduct).map(([groupKey, teeth]) => {
                  ...
                  return (
                    <div key={`prep-pontic-group-${groupKey}`} className="relative mt-3">
                      <FloatingActionIcons
                        arch="mandibular"
                        visible={showActionsMand}
                        onAttach={() => setShowAttachModal(true)}
                        onRush={() => handleOpenRushModal("mandibular", isFixedCategory(categoryName) ? `fixed_${firstToothNumber}` : `prep_${firstToothNumber}`)}
            ...
            return (
              <div key="initial-removables-mandibular" className="relative mt-3">
                <FloatingActionIcons
                  ...
                  onAttach={() => setShowAttachModal(true)}
                  onRush={() => handleOpenRushModal("mandibular", `prep_${cardTeeth[0]}`)}
              ...
              return (
                <div key={ap.id} className="relative mt-3">
                  <FloatingActionIcons
                    ...
                    onAddOns={() => handleOpenAddOnsModal("mandibular", getToothProduct("mandibular", apRepTn)?.id?.toString() || `prep_${apRepTn}`, apRepTn)}
                    onAttach={() => setShowAttachModal(true)}
                    onRush={() => handleOpenRushModal("mandibular", apProductKey)}
                  ...
                  {ap.expanded && (
                    <div className={`border-t border-[#d9d9d9] p-2.5 sm:p-4 bg-white space-y-3 max-h-[600px] overflow-y-auto scrollbar-blue`}>
                      {cardTeeth.length === 0 ? (
                        ...
                        const isCardRemovables = isRemovableCategory(cardCategoryName);
                        const repTn = cardTeeth[0];
                        const toothProduct = getToothProduct("mandibular", repTn);
