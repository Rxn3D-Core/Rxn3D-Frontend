...
interface CreateTeethShadeGroupModalProps {
  isOpen: boolean
  onClose: () => void
  onChanges: (hasChanges: boolean) => void
}

export function CreateTeethShadeGroupModal({ isOpen, onClose, onChanges }: CreateTeethShadeGroupModalProps) {
  const [isMaximized, setIsMaximized] = useState(false)
  const [groupDetailsEnabled, setGroupDetailsEnabled] = useState(true)
  const [groupName, setGroupName] = useState("")
  const [selectTeethShadeOpen, setSelectTeethShadeOpen] = useState(true)
  ...
  // Mock teeth shades
  const mockTeethShades = [
    { id: 1, brand: "VITA Zahnfabrik", system: "VITA Classical", selected: false },
    { id: 2, brand: "VITA Zahnfabrik", system: "Artic Shade", selected: false },
    { id: 3, brand: "VITA Zahnfabrik", system: "VITA 3D-Master", selected: false },
    { id: 4, brand: "Ivoclar Vivadent", system: "ChromaScop®", selected: false },
  ]

  const [teethShades, setTeethShades] = useState(mockTeethShades)

  // Track changes
  useEffect(() => {
    const hasChanges = groupName.trim() !== "" || teethShades.some((shade) => shade.selected)
    onChanges(hasChanges)
  }, [groupName, teethShades, onChanges])
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setSelectTeethShadeOpen(true)
      setLinkToProductsOpen(false)
      setLinkToExistingGroupsOpen(false)
      setVisibilityManagementOpen(false)
      setSearchQuery("")
      setTeethShades(mockTeethShades)
    }
  }, [isOpen])

  const toggleTeethShadeSelection = (id: number) => {
    setTeethShades(teethShades.map((shade) => (shade.id === id ? { ...shade, selected: !shade.selected } : shade)))
  }
  ...
  // Filter teeth shades based on search query
  const filteredTeethShades = teethShades.filter(
    (shade) =>
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <DialogTitle className="text-xl font-bold">Create new teeth shade group</DialogTitle>
        ...
        <div className="overflow-y-auto max-h-[80vh] p-6 space-y-6">
          ...
          {/* Select Teeth Shade Section */}
          <Collapsible open={selectTeethShadeOpen} onOpenChange={setSelectTeethShadeOpen}>
            <CollapsibleTrigger className="flex items-center justify-between w-full border-t border-b py-4">
              <div className="flex items-center gap-2">
                <span className="font-medium">Select Teeth shade</span>
                <div className="rounded-full bg-gray-200 text-gray-600 w-5 h-5 flex items-center justify-center text-xs">
                  ?
                </div>
              </div>
              <ChevronDown className={`h-5 w-5 transition-transform ${selectTeethShadeOpen ? "rotate-180" : ""}`} />
            </CollapsibleTrigger>
