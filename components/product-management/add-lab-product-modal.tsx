...
import { collectJawPhotosStepErrors } from "@/lib/product-details-step-validation"
import {
...
} from "@/lib/product-advance-fields-form"
import {
...
import type { ProductSaveResult } from "@/contexts/product-products-context"
...
function buildSectionsStateFromProduct(editingProduct: any) {
  ...
  return {
    ...
    advanceField: slipFlagFromApi((editingProduct as { has_advance_field?: unknown }).has_advance_field, true),
...
interface AddLabProductModalProps {
  ...
  updateProduct: (id: number, payload: any, releasingStageIds?: (string | number)[]) => Promise<ProductSaveResult>;
  createProduct: (payload: any, releasingStageIds?: (string | number)[]) => Promise<ProductSaveResult>;
  isUpdating: boolean;
  isCreating: boolean;
  /** When provided, modal can reload full detail after save and notify parent without closing */
  getProductDetail?: (id: number) => Promise<unknown>;
...
export function AddLabProductModal({ 
  ...
  const [advanceFieldHost, setAdvanceFieldHost] = useState<AdvanceFieldModalHostState>({ open: false })
  ...
  useAdvanceFields(
    {
      ...
      status: "Active",
      ...(typeof customerId === "number" && customerId > 0 ? { customer_id: customerId } : {}),
    },
    { enabled: isOpen },
  )
  ...
  const watchedShowJawPhoto = useWatch({ control, name: "show_jaw_photo" })
  const watchedHasGradeBasedPricing = watch("has_grade_based_pricing")

  // Clear / refresh jaw photo validation errors as slots or the toggle change
  useEffect(() => {
    setManualSlipRelationErrors((prev) => {
      const prevJaw = prev.filter((e) => e.field.startsWith("jaw_photos"))
      if (prevJaw.length === 0) return prev
      const without = prev.filter((e) => !e.field.startsWith("jaw_photos"))
      const nextJaw = collectJawPhotosStepErrors(
        { show_jaw_photo: watchedShowJawPhoto } as Record<string, unknown>,
        jawPhotos,
      )
      if (
        nextJaw.length === prevJaw.length &&
        nextJaw.every((e, i) => e.field === prevJaw[i]?.field && e.message === prevJaw[i]?.message)
      ) {
        return prev
      }
      return [...without, ...nextJaw]
    })
  }, [jawPhotos, watchedShowJawPhoto])
  const masterGradesForValidation = useMemo(
    () =>
      (Array.isArray(grades) ? grades : (grades as { data?: unknown[] })?.data || []).map(
        (g: { id?: number | string; sequence?: number }) => ({
  ...
  const enforceLabPricing = userRole !== "superadmin"
  ...
  const handleNext = async () => {
    ...
    if (activeTab === "details") {
      ...
      const jawErrs = collectJawPhotosStepErrors(record, jawPhotos)
      setManualSlipRelationErrors((prev) => {
        const without = prev.filter((e) => !e.field.startsWith("jaw_photos"))
        return [...without, ...jawErrs]
      })
      if (jawErrs.length > 0) {
        toast({
          title: "Validation Error",
          description: jawErrs[0].message,
          variant: "destructive",
        })
        return
      }
  ...
  const flattenErrors = (errors: any, prefix = ""): ValidationFieldError[] => {
    const errorList: ValidationFieldError[] = []
  ...
  const flattenBackendErrors = (errors: any, prefix = ""): ValidationFieldError[] => {
    if (!errors || typeof errors !== "object") {
      return []
    }

    const backendErrors: ValidationFieldError[] = []
      ...
      if (Array.isArray(value)) {
        value.forEach((item) => {
          if (typeof item === "string") {
            backendErrors.push({ field: fieldName, message: item })
          } else if (item && typeof item === "object") {
            if ("message" in item && typeof (item as any).message === "string") {
              backendErrors.push({ field: fieldName, message: (item as any).message })
            } else {
              backendErrors.push(...flattenBackendErrors(item, fieldName))
            }
          }
        })
      } else if (typeof value === "string") {
        backendErrors.push({ field: fieldName, message: value })
      } else if (value && typeof value === "object") {
        backendErrors.push(...flattenBackendErrors(value, fieldName))
      }
    }
  ...
  useEffect(() => {
    if (!isValid && errors && Object.keys(errors).length > 0) {
      ...
      setClientValidationErrors(errorList)
    } else {
      setClientValidationErrors([])
    }
  }, [isValid, errors])
  ...
  useEffect(() => {
    setManualSlipRelationErrors((prev) =>
      prev.filter((e) => {
        ...
        if ((e.field === "grades" || e.field.startsWith("grades.")) && !sections.grades) return false
        if ((e.field === "stages" || e.field.startsWith("stages.")) && !sections.stages) return false
        if (
          (e.field === "tooth_count_variations" || e.field.startsWith("tooth_count_variations.")) &&
          !sections.variation
        ) {
          return false
        }
    ...
    if (isOpen && editingProduct) {
      ...
      function mapWithStatus(arr: any[], idKey: string) {
        ...
        return arr.map((item, idx) => {
          ...
          let rawPrice: unknown =
            item.price ??
            item.pivot?.price ??
            (item as { lab_material?: { price?: unknown } }).lab_material?.price ??
          ...
          if (
            ...
            rawPrice = retentionPriceByRid.get(Number(entityId))
          }
      ...
      function mapExtractions(arr: any[]) {
        ...
        return arr.map((item, idx) => ({
          ...
          is_required: (item.is_required === "Yes" ? "Yes" : "No") as "Yes" | "No",
          is_optional: (item.is_optional === "Yes" ? "Yes" : "No") as "Yes" | "No",
      ...
      // Find category_id from subcategory_id if available
      ...
      if (mappedGrades.length > 0) {
        ...
        const defaultGrade = mappedGrades.find((g: any) => {
          const price = typeof g.price === "number" ? g.price : parseFloat(g.price)
          return g.is_default === "Yes" && !isNaN(price) && price > 0
        })
        if (defaultGrade) {
          const price = typeof defaultGrade.price === "number" ? defaultGrade.price : parseFloat(defaultGrade.price)
          basePrice = price.toString()
        } else {
          // If no default grade with price, find any grade with price (price must be > 0)
          const gradeWithPrice = mappedGrades.find((g: any) => {
            const price = typeof g.price === "number" ? g.price : parseFloat(g.price)
  ...
  const onSubmit = async (data: ProductCreateForm) => {
    ...
    const fail = firstProductModalValidationFailure({ ...data } as Record<string, unknown>, {
      sections: {
    ...
    setManualSlipRelationErrors(fail?.allErrors ?? [])
    if (fail) {
      toast({
        title: "Validation Error",
        description: fail.errors[0].message,
        variant: "destructive",
      })
    ...
    const hydrateAfterSaveLab = async (snapshot: Record<string, unknown> | null) => {
      ...
      if (!getProductDetail) return snapshot
      const detail = await getProductDetail(rid)
    ...
    const mergedPersisted = await hydrateAfterSaveLab(saveResult.productSnapshot)
    if (mergedPersisted && onProductPersisted) {
      onProductPersisted(mergedPersisted)
    }

    toast({
      title: "Success",
      description: editingProduct?.id ? "Product updated successfully." : "Product created successfully.",
      variant: "default",
    })
    ...
    clearValidationErrors()
    try {
      setInitialFormValues(structuredClone(data) as ProductCreateForm)
    } catch {
      setInitialFormValues(JSON.parse(JSON.stringify(data)) as ProductCreateForm)
    }
  ...
  const handleFormSubmit = handleSubmit(
    ...
    (errors) => {
      // Handle validation errors - show toast notification and jump to the owning tab
      ...
      if (errorMessages.length > 0) {
        ...
        const tabOrder = Object.keys(
          // prefer PRODUCT_MODAL_TAB_FIELD_PREFIXES order via tabHasValidationErrors
          {
            details: true,
            variation: true,
            grades: true,
            ...
            material: true,
            addOns: true,
            toothChartConfigurations: true,
            advanceFields: true,
            visibility: true,
          },
        )
        const jumpTab = tabOrder.find((tabId) => tabHasValidationErrors(tabId, errorMessages))
        if (jumpTab) {
          setVisibleTabs((prev) => new Set([...prev, jumpTab]))
  ...
  const SECTION_TOGGLE_KEYS = ["variation", "grades", "stages", "impressions", "gumShade", "teethShade", "material", "addOns", "retention", "advanceField", "extractions"] as const
  ...
  const handleUpdateSection = async () => {
    ...
    try {
      ...
      const failSection = firstProductModalValidationFailure(formData as Record<string, unknown>, {
        sections: {
          impressions: sections.impressions,
          gumShade: sections.gumShade,
          teethShade: sections.teethShade,
        ...
        enforceLabPricing,
        isSingleStage: formData.is_single_stage === "Yes",
        isSuperAdmin: userRole === "superadmin",
        masterGrades: masterGradesForValidation,
        jawPhotos,
      })
      setManualSlipRelationErrors(failSection?.allErrors ?? [])
      if (failSection) {
        toast({
          title: "Validation Error",
          description: failSection.errors[0].message,
          variant: "destructive",
        })
        setVisibleTabs((prev) => new Set([...prev, failSection.tabId]))
        setActiveTab(failSection.tabId)
        return
      }
      ...
      try {
        const outcome = await updateProduct(editingProduct.id, payload, releasingIds)
        if (!outcome.success) {
          toast({
            title: "Error",
        ...
        let mergedPersistedTab: Record<string, unknown> | null = outcome.productSnapshot
        const rid =
          editingProduct?.id ??
          (outcome.productSnapshot?.id !== undefined && outcome.productSnapshot?.id !== null
            ? typeof outcome.productSnapshot.id === "number"
              ? outcome.productSnapshot.id
              : typeof outcome.productSnapshot.id === "string" &&
                  /^\d+$/.test(String(outcome.productSnapshot.id).trim())
                ? Number.parseInt(String(outcome.productSnapshot.id), 10)
                : null
            : null)
        if (typeof rid === "number" && Number.isFinite(rid) && getProductDetail) {
          const detail = await getProductDetail(rid)
          if (detail && typeof detail === "object") {
            mergedPersistedTab = detail as Record<string, unknown>
          }
        }
        if (mergedPersistedTab && onProductPersisted) {
        ...
        <DialogContent
          className={`p-0 gap-0 transition-all duration-300 ease-in-out flex flex-col ${
            isMaximized 
              ...
              : "w-full max-w-[900px] h-[90vh] max-h-[90vh]"
          } bg-white`}
        >
                  ...
                  <div className="flex">
                    {tabs.map((tab) => {
                      ...
                      const isSectionOff = tab.sectionKey
                        ...
                        : false
                      const hasTabErrors = tabHasValidationErrors(tab.id, validationErrors)
