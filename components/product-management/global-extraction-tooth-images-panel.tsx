...
import {
  useDeleteGlobalExtractionToothImage,
  useGlobalExtractionToothImages,
  useUpsertGlobalExtractionToothImages,
...
interface GlobalExtractionToothImagesPanelProps {
  extractionId: number
}

export function GlobalExtractionToothImagesPanel({ extractionId }: GlobalExtractionToothImagesPanelProps) {
  ...
  const { data: images, isLoading, isFetching, error, refetch } = useGlobalExtractionToothImages(extractionId)
  const upsertMutation = useUpsertGlobalExtractionToothImages()
  const deleteMutation = useDeleteGlobalExtractionToothImage()
  ...
  const assignToTooth = useCallback(
    ...
    [extractionId, upsertMutation, validateAndReadFile],
  )
  ...
  return (
    <div className="flex flex-col gap-4 pb-1">
      ...
      {/*
        md+: 16 columns × 2 rows (teeth 1–16, then 17–32) for wider cells.
        Below md: 8 then 4 columns so small screens stay usable without microscopic tiles.
      */}
        ...
        {UNIVERSAL_TEETH.map((n) => {
          ...
          return (
            <div
              ...
              className={cn(
                /* Slightly landscape slots so tall/narrow clinical shots scale by width first (still object-contain, never stretched). */
