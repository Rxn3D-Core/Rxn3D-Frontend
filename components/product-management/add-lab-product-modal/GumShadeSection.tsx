...
export function GumShadeSection({
  ...
  const toggleBrandExpansion = (brandId: number) => {
    ...
    const hasSelectedShades = brand && Array.isArray(brand.shades) && brand.shades.some(
      (shade: GumShade) => watchedGumShades.some((gs: WatchedGumShade) => gs.gum_shade_id === shade.id)
    )
    ...
    if (!isCurrentlyExpanded && !hasSelectedShades) {
      ...
      if (brand && Array.isArray(brand.shades) && brand.shades.length > 0) {
        ...
        brand.shades.forEach((shade: GumShade) => {
          const existingShade = currentList.find((gs: WatchedGumShade) => gs.gum_shade_id === shade.id)
          if (!existingShade) {
            maxSequence += 1
            newGumShades.push({
