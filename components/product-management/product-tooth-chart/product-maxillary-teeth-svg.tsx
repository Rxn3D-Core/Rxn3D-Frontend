/**
 * Product library tooth chart clone — forked from `components/maxillary-teeth-svg.tsx`.
...
interface ProductMaxillaryTeethSVGProps {
...
export const ProductMaxillaryTeethSVG: React.FC<ProductMaxillaryTeethSVGProps> = ({
  selectedTeeth,
  onToothClick,
  className = '',
  ...
  // Re-render once after opening so svgRef + popover position are ready inside the modal.
  ...
  const getToothBaseRectStyle = (toothNumber: number): React.CSSProperties => {
    ...
    // Retention assignment replaces extraction base (combined product preview).
    ...
    // Missing-tooth overlay replaces the default graphic when no catalog image URL exists.
  ...
  return (
    <>
      ...
      {effectiveShowRetentionPopover && retentionPopoverTooth !== null && (onSelectRetentionType || onSelectRetentionOption || onSelectExtractionStatus) && typeof window !== 'undefined' && (() => {
      ...
      >
        ...
        <svg ref={svgRef} className="w-full h-auto" viewBox={`0 0 695 ${showCheckboxes ? "158" : "139"}`} fill="none" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink">
          {toothMapping.map(({ tooth, x, width, pattern }) => {
            ...
            const showRetentionImage = isToothShowingRetentionImage(tooth)
            const showS3Image = isToothShowingS3Image(tooth) && !!s3Url && !showRetentionImage
            ...
            return (
              <g key={tooth}>
                ...
                {showRetentionImage && renderRetentionImageOverlay(tooth, x, width)}
                {!showS3Image && !showRetentionImage && renderExtractionImageOverlay(tooth, x, width)}
