...
test("buildPaperSlipPrintSlipVM maps header, footer, notes, and chart-ready arch state", () => {
  ...
  assert.equal(slip.header.dueDate, "08/14/26 @ 4:00 PM");
...
test("buildPaperSlipPrintSlipVM keeps only the latest slip note for print", () => {
  const slip = buildPaperSlipPrintSlipVM({
    notes: [
  ...
  assert.deepEqual(slip.notes, ["Latest note"]);
});
