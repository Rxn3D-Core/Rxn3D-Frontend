...
test("advance multi-select option ids resolve to option names", () => {
  const vm = buildVirtualSlipVM({
    case: { patient_name: "Test" },
    products: [
      {
        ...
        advance_fields: [
          {
            field_name: "Metal Design",
            advance_field_value: "[121,120]",
            display_value: "121, 120",
            selections: [
              {
                field_name: "Metal Design",
                value: "121, 120",
            ...
            advance_field: {
              ...
              field_type: "checkbox",
              options: [
                { id: 120, name: "Full metal" },
                { id: 121, name: "Porcelain fused" },
              ],
            },
          },
        ],
  ...
  const metal = vm.arches.maxillary?.products?.[0]?.advanceFields?.find(
    (row) => row.label === "Metal Design"
  );
  assert.equal(metal?.value, "Porcelain fused, Full metal");
});

test("teeth shade shows system and code without brand", () => {
  const vm = buildVirtualSlipVM({
    case: { patient_name: "Test" },
    products: [
      {
        ...
        name: "All on X",
        teeth_shade: { name: "A1" },
        teeth_shade_brand: { name: "Zahnfabrik", system_name: "Vita Classical" },
        gum_shade: { name: "Light Pink" },
        gum_shade_brand: { name: "Acrylic Base Shades", system_name: "Standard Pink" },
      },
    ],
  });

  const product = vm.arches.maxillary?.products?.[0];
  assert.equal(product?.teethShade, "Vita Classical - A1");
  assert.equal(product?.gumShade, "Acrylic Base Shades - Standard Pink - Light Pink");
});
