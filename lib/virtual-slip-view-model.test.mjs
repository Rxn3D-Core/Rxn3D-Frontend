...
test("buildVirtualSlipVM uses tooth_chart_preload for arch tooth chart", () => {
  ...
  assert.ok(arch);
  // Top chart: extractions only (status teeth), not product teeth_selection.
  assert.equal(arch.extractionDisplay.extractionImagesByCode.MT[2], "https://example.com/missing-2.png");
  assert.equal(arch.extractionDisplay.toothExtractionMap[7], undefined);
...
test("buildVirtualSlipVM arch chart skips product teeth; product summary keeps teeth_selection", () => {
  const vm = buildVirtualSlipVM({
    case: { patient_name: "Test" },
    products: [
      {
  ...
  const arch = vm.arches.maxillary;
  assert.ok(arch);
  assert.equal(arch.extractionDisplay.toothExtractionMap[8], undefined);
