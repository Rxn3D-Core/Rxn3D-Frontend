...
test("buildVirtualSlipInitialState hydrates opposite_impressions onto the other jaw", () => {
  const state = buildVirtualSlipInitialState([
    {
      id: 1,
      type: "Upper",
      teeth_selection: "5",
      category: { name: "Removable Restorations" },
      product: { id: 200, name: "Flexible/Valplast" },
      impressions: [
        {
          impression_id: 3,
          quantity: 1,
          impression: { id: 3, code: "ALG", name: "Alginate" },
        },
      ],
  ...
  assert.equal(state.selectedImpressions.maxillary.length, 1);
  assert.equal(state.selectedImpressions.maxillary[0].code, "ALG");
  assert.equal(state.selectedImpressions.mandibular.length, 1);
  assert.equal(state.selectedImpressions.mandibular[0].code, "ALG");
  assert.equal(state.selectedImpressions.mandibular[0].qty, 1);
});

test("buildVirtualSlipInitialState preloads removable IPS teeth shade + guide", () => {
  const state = buildVirtualSlipInitialState([
    {
      type: "Upper",
      teeth_selection: "7,8,9,10",
      category: { name: "Removables Restorations" },
      teeth_shade: { id: 55, name: "A2" },
      teeth_shade_brand: {
        id: 9,
        name: "Ivoclar Vivadent",
        system_name: "IPS Shade System",
      },
      gum_shade: { id: 44, name: "Standard Pink" },
      gum_shade_brand: { id: 3, name: "St. George", system_name: "St George Acrylic Base Shades" },
      product: {
        id: 88,
        name: "Flipper / Stayplate",
        has_teeth_shade: "Yes",
        has_gum_shade: "Yes",
      },
    },
  ...
  assert.equal(state.selectedShadeGuide, "IPS Shade System");
  assert.equal(state.selectedShades["prep_7_maxillary_tooth_shade"], "A2");
  assert.equal(state.selectedShades["prep_10_maxillary_tooth_shade"], "A2");
  assert.ok(state.completedFields.maxillary_7.includes("teeth_shade"));
  assert.ok(state.completedFields.maxillary_10.includes("teeth_shade"));

  const teeth = JSON.parse(state.fieldValues.maxillary_7.teeth_shade);
  assert.equal(teeth.name, "A2");
  assert.equal(teeth.teeth_shade_id, 55);
  assert.equal(teeth.brand_id, 9);

  const gum = JSON.parse(state.fieldValues.maxillary_8.gum_shade);
  assert.equal(gum.name, "Standard Pink");
  assert.equal(gum.brand_id, 3);
});
