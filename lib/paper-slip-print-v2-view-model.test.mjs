...
test("buildPaperSlipPrintV2SlipVM dueDate uses delivery.final_date", () => {
  const extras = extractPaperSlipPrintV2Extras(makeRecord());
  const slip = buildPaperSlipPrintV2SlipVM(
    makeDetails({
      delivery: {
  ...
  assert.equal(slip.vm.header.dueDate, "08/14/26");
  assert.equal(slip.vm.header.deliveryTime, "4:00 PM");
});

test("buildPaperSlipPrintV2SlipVM deliveryTime uses lab default when slip time is midnight", () => {
  const extras = extractPaperSlipPrintV2Extras(makeRecord());
  const slip = buildPaperSlipPrintV2SlipVM(
    makeDetails({
      delivery: { delivery_time: "2026-08-18T00:00:00.000000Z" },
    }),
    extras,
    { defaultDeliveryTime: "17:00" },
  );

  assert.equal(slip.vm.header.deliveryTime, "5:00 PM");
});

test("buildPaperSlipPrintV2SlipVM keeps only the latest slip note for print", () => {
  const extras = extractPaperSlipPrintV2Extras(makeRecord());
  const slip = buildPaperSlipPrintV2SlipVM(
    {
      ...makeDetails(),
  ...
  assert.equal(slip.vm.notes, "Latest note");
});
