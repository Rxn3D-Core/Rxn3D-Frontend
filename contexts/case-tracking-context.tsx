  ...
  const fetchCasePans = useCallback(async (customerId?: number) => {
    ...
    try {
      ...
      const transformedPans = (data.data?.data || data.data || []).map((pan: any) => ({
        ...
        linkedProducts: [],
        availableCodes: {
          used: pan.casepan_number_availability?.used || 0,
          total: pan.casepan_number_availability?.total || pan.quantity || 0,
          overcapacity: (pan.casepan_number_availability?.used || 0) > (pan.casepan_number_availability?.total || pan.quantity || 0),
          full: (pan.casepan_number_availability?.available || 0) === 0,
        },
